...
const registerUser = async (req, res) => {
    try {
        const { error } = validation_1.validateRegister.validate(req.body);
        if (error) {
            const errorMsg = error.details.map((er) => er.message).join(", ");
            res.status(400).send({ success: false, message: errorMsg });
            return;
        }
...
const loginUser = async (req, res) => {
    try {
        const { error } = validation_1.validateLogin.validate(req.body);
        if (error) {
            res
                .status(400)
